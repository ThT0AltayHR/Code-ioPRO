...
/**
 * SubAgentFragment — AI sub-agent management panel.
 * Shows real-time agent activity, prompt chains, file creation logs.
 */
public class SubAgentFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedState) {
        ...
        agentWebView.setWebViewClient(new WebViewClient() {
            @Override public void onPageFinished(WebView v, String url) { injectInitialState(); }
        });
    ...
    private void injectInitialState() {
        ...
        agentWebView.evaluateJavascript(
            "if(window.initAgentUI){window.initAgentUI('" + model + "','" + mode + "'," + json + ");}", null);
    }
    ...
    private int estimateAgentCount(String desc) {
        ...
        if (w < 20) return 1; if (w < 50) return 2; if (w < 100) return 3;
    ...
    private void runAgentSimulation(AgentSession main) {
        executor.execute(() -> {
            try {
                ...
                main.status = "running"; main.currentTask = "Sistem promptu oluşturuluyor";
                updateUI(); Thread.sleep(1200);
                main.addLog("✅ Prompt hazır. Görevler dağıtılıyor...");
                for (int i = 1; i < activeSessions.size(); i++) {
                    AgentSession sub = activeSessions.get(i);
                    Thread.sleep(400);
                    sub.status = "running"; sub.currentTask = "Görev alındı, çalışıyor...";
                    sub.addLog("🚀 Başlatıldı (" + sub.model + ")");
                    updateUI();
                }
                Thread.sleep(2000);
                main.addLog("📁 Dosya yapısı planlandı"); main.filesCreated = 3;
                updateUI();
    ...
    private void updateUI() {
        mainHandler.post(() -> {
            if (agentWebView != null)
                agentWebView.evaluateJavascript(
                    "if(window.updateSessions){window.updateSessions(" + buildSessionsJson() + ");}", null);
        });
    }
    ...
    class AgentBridge {
        ...
        @JavascriptInterface public void setMode(String mode) { prefs.edit().putString("agent_mode", mode).apply(); }
        @JavascriptInterface public String getActiveMode() { return prefs.getString("agent_mode", "economy"); }
        @JavascriptInterface public void clearSessions() { activeSessions.clear(); mainHandler.post(SubAgentFragment.this::updateUI); }
    }
}
